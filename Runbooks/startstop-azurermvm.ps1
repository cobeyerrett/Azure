...
if (($power -like "start") -or ($power -like "stop") ) {
    Write-output "The VMs are set to $power"
}
else{
    Write-Error "The power parameter must be Start or Stop"
}
...
try {
    ...
    Write-output "No Such Subscription found" -ForegroundColor Yellow
    exit
}
...
if (!$power){Write-output "No powerstate specified. Use -Power start|stop"}
if (!$azureResourceGroup){
    Write-output "No Azure Resource Group specified. Use -azureResourceGroup 'ResourceGroupName'"}

    Write-output "Enumerating VM's from AzureRM in Resource Group '"$azureResourceGroup "'"
...
if ($power -eq 'start') {
    Write-output "Starting VM's "$vmstoppedlist " in Resource Group "$azureResourceGroup       
...
if ($power -eq 'stop') {
     Write-output "Stopping VM's "$vmrunninglist " in Resource Group "$azureResourceGroup       
...
startstop-azurermvm -power $power -azureResourceGroup $ResourceGroup